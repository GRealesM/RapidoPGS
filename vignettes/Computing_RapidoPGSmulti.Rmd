...
```{r eval=FALSE}
model.LDmat <- rapidopgs_multi(ds ,trait="cc", LDmatrices = "ukbb" , N_LD=362320)
```

Where `N_LD` corresponds to the sample size of the panel use to compute the LD matrices (362,320 individuals), and `LDmatrices` points at the directory where our LD matrices live. 
...
Here, instead of `LDmatrices` and `N_LD`, we just need to set `reference` to point to the directory in which our panel lives. 
